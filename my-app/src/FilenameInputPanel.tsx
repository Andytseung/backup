...
const FilenameInputPanel: React.FC<FilenameInputPanelProps> = ({ onFilesChange, onParaChange, onAlignedChange }) => {
...
const toggleAligned = () => {
  ...
  const [miniMapOpen, setMiniMapOpen] = useState<boolean[]>(files.map(() => true));
  ...
  const autoGenerateFiles = () => {
    ...
    if(files.length >= 4){
      ...
      const isDuplicate = last4.every((file,i) => {
        const timeStr = startTimes[i];
        const y = parseInt(timeStr.slice(0,4),10);
        const m = parseInt(timeStr.slice(4,6),10);
        const d = parseInt(timeStr.slice(6,8),10);
        const h = timeStr.slice(8,10);
        return file.year === y && file.month === m && file.day === d && file.time === h;
      });
    ...
    setExpandedIndex(files.length);
    setMiniMapOpen(open => [...open, ...newFiles.map(() => false)]);
  };
...
export default FilenameInputPanel;
